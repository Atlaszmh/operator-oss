...
- **Shipping shows its work** — Ship is four slow steps: your test suite against the assembled branch, the merge, catching every other live feature branch up, then the push. On a real project that's minutes. Each step names itself as it starts and says how long it took as it lands, streamed while it happens, so a slow ship is legible and a stuck one is attributable to a step rather than to a disabled button.
- **Keys** — every task and feature gets a JIRA-style identifier from its project's prefix, with a letter in the middle saying which it is: `TME-T42` is a task, `TME-F41` a feature. So there's something to paste into a commit message or send to someone, and its scope is readable at a glance. Search and ⌘K both match on it. The prefix is derived from the project name and editable in the project's context editor; changing it re-keys everything in that project at once.
...
- **Autopilot interrupts you only when it means it** — a member task whose turn ends is handing off to the gate, not to you, so it no longer announces "needs your input" for work that is about to merge itself unattended. The flag is raised when autopilot genuinely stops — and a gate that can't reach a verdict for three whole test windows counts as stopped, so finished work can't sit parked and invisible behind a reviewer that isn't coming back.
