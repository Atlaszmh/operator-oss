...
export const SUGGEST_TASK = {
  ...
  params: {
    ...
    feature:
      "Name (or id) of the feature this task belongs to. Group a multi-task breakdown under one feature so the tasks " +
      "share context and can ship together — call `suggest_feature` first to write that shared context. An unknown " +
      "name creates a bare feature with no context. Omit for standalone work.",
  },
...
export const SUGGEST_FEATURE = {
  ...
  description:
    "Declare a FEATURE in this project's orchestrator: a named group of related tasks that share context and ship together " +
    "(the layer between a project and its tasks). Use it when you're breaking down one coherent piece of work rather than " +
    "listing unrelated errands — call this FIRST with the shared spec, then pass its name as `feature` on each `suggest_task`. " +
    "The context you give here is prepended to every member task's session, so write the data model, constraints and decisions " +
    "ONCE here instead of repeating them in each task description. Calling this with the name of an existing feature UPDATES " +
    "that feature rather than creating a duplicate, so it's safe to re-run. Does not create git branches — that's the user's call. " +
    'To CHAIN features (build B after A ships), pass after: ["A"] on B.',
  params: {
    name: 'Short feature name, unique within the project, e.g. "Billing v2". This is what `suggest_task` references.',
    description: "One line describing what the feature is, for the task list header.",
    context:
      "The shared spec every task in this feature needs: data model, constraints, decisions, relevant files. Prepended to " +
      "each member task's prompt. Keep it spec-sized — it is re-sent on every turn of every task in the feature.",
    after:
