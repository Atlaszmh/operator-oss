...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Whitelist. `branch`/`base_sha`/`merged_at` are deliberately absent: those are
  // git state, owned by the /branch and /ship routes, which hold the guards. A
  // PATCH that could set `branch` directly would let the client re-point live
  // worktrees' merge target without any of them.
